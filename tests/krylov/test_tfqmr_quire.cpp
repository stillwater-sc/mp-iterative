// TFQMR (Transpose-Free Quasi-Minimal Residual) with quire (exact dot
// product) accumulation: mirrors test_cgs_quire.cpp / test_minres_quire.cpp
// but exercises itl::tfqmr. (MTL5 itself must not depend on Universal.)
//
// Two checks, no external test framework (matching the repo's lightweight
// style; returns non-zero on failure):
//   1. TFQMR with the default (unspecified) Accumulator matches the
...
bool tfqmr_default_accumulator_ok() {
    ...
    int err = itl::tfqmr(A, x, b, pc, iter); // no explicit Accumulator -- must be unaffected
    if (err != 0) {
        std::cerr << "TFQMR (default Accumulator) did not converge, err=" << err << '\n';
        return false;
    }
    ...
    for (std::size_t i = 0; i < n; ++i) {
        if (std::abs(r(i) - b(i)) > 1e-8) {
            std::cerr << "TFQMR (default Accumulator) residual component " << i
                      << " off: " << r(i) << " vs " << b(i) << '\n';
            return false;
        }
    }
...
bool tfqmr_quire_beats_naive_posit32() {
    ...
    itl::tfqmr(A, x_naive, b, pc, iter_naive); // default Accumulator = naive posit32
...
int main() {
    ...
    if (!tfqmr_default_accumulator_ok())      ++failures;
    if (!tfqmr_quire_beats_naive_posit32())   ++failures;

    if (failures == 0) std::cout << "test_tfqmr_quire passed\n";
