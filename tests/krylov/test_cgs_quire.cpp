// CGS (Conjugate Gradient Squared) with quire (exact dot product)
// accumulation: mirrors test_bicgstab_quire.cpp / test_bicgstab_ell_quire.cpp
// but exercises itl::cgs. (MTL5 itself must not depend on Universal.)
//
// Two checks, no external test framework (matching the repo's lightweight
// style; returns non-zero on failure):
//   1. CGS with the default (unspecified) Accumulator matches the
...
bool cgs_default_accumulator_ok() {
    ...
    int err = itl::cgs(A, x, b, pc, iter); // no explicit Accumulator -- must be unaffected
    if (err != 0) {
        std::cerr << "CGS (default Accumulator) did not converge, err=" << err << '\n';
        return false;
    }
    ...
    for (std::size_t i = 0; i < n; ++i) {
        if (std::abs(r(i) - b(i)) > 1e-8) {
            std::cerr << "CGS (default Accumulator) residual component " << i
                      << " off: " << r(i) << " vs " << b(i) << '\n';
            return false;
        }
    }
...
bool cgs_quire_beats_naive_posit32() {
    ...
    itl::cgs(A, x_naive, b, pc, iter_naive); // default Accumulator = naive posit32

    vec::dense_vector<Posit> x_quire(n, Posit(0.0));
    itl::basic_iteration<Posit> iter_quire(b, 200, Posit(1e-6));
    itl::cgs<mat::dense2D<Posit>, vec::dense_vector<Posit>, vec::dense_vector<Posit>,
...
int main() {
    ...
    if (!cgs_default_accumulator_ok())      ++failures;
    if (!cgs_quire_beats_naive_posit32())   ++failures;

    if (failures == 0) std::cout << "test_cgs_quire passed\n";
