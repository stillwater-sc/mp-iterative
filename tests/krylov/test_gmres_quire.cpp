// GMRES with quire (exact dot product) accumulation: the MTL5 + Universal
// composition this repo exists for. Mirrors test_bicgstab_quire.cpp.
...
//   1. GMRES with the default (unspecified) Accumulator matches the
//      unmodified baseline on a small system.
//   2. GMRES handles a trivial 1x1 system.
...
bool gmres_default_accumulator_ok() {
    ...
    int err = itl::gmres(A, x, b, pc, iter, /*restart=*/10); // no explicit Accumulator -- must be unaffected
    if (err != 0) {
        std::cerr << "GMRES (default Accumulator) did not converge, err=" << err << '\n';
        return false;
    }
    ...
    for (std::size_t i = 0; i < 3; ++i) {
        if (std::abs(r(i) - b(i)) > 1e-8) {
            std::cerr << "GMRES (default Accumulator) residual component " << i
                      << " off: " << r(i) << " vs " << b(i) << '\n';
            return false;
        }
    }
...
// --- Edge case: GMRES on a trivial 1x1 system ---
bool gmres_1x1_ok() {
    ...
    if (err != 0) {
        std::cerr << "GMRES (1x1) did not converge, err=" << err << '\n';
        return false;
    }
    if (std::abs(x(0) - 0.5) > 1e-8) {
        std::cerr << "GMRES (1x1) wrong solution: " << x(0) << " vs 0.5\n";
        return false;
    }
    return true;
}
...
bool gmres_quire_beats_naive_posit32() {
    ...
    itl::gmres(A, x_naive, b, pc, iter_naive, /*restart=*/10); // default Accumulator = naive posit32

    vec::dense_vector<Posit> x_quire(n, Posit(0.0));
    itl::basic_iteration<Posit> iter_quire(b, 200, Posit(1e-6));
    itl::gmres<mat::dense2D<Posit>, vec::dense_vector<Posit>, vec::dense_vector<Posit>,
            itl::pc::identity<mat::dense2D<Posit>>, itl::basic_iteration<Posit>, Quire>(
        A, x_quire, b, pc, iter_quire, /*restart=*/10);
...
int main() {
    ...
    if (!gmres_default_accumulator_ok())      ++failures;
    if (!gmres_1x1_ok())                      ++failures;
    if (!gmres_quire_beats_naive_posit32())   ++failures;

    if (failures == 0) std::cout << "test_gmres_quire passed\n";
