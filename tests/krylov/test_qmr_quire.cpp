// QMR with quire (exact dot product) accumulation: the MTL5 + Universal
// composition this repo exists for. Mirrors test_cg_quire.cpp's structure
// for QMR (see mtl5#276/#277, mp-iterative#22).
//
// Three checks, no external test framework (matching the repo's lightweight
// style; returns non-zero on failure):
//   1. QMR with the default (unspecified) Accumulator matches the unmodified
//      baseline on a small non-symmetric system.
//   2. QMR handles a trivial 1x1 system.
//   3. The actual point: quire-accumulated dot()/mult() improves posit32
...
bool qmr_default_accumulator_ok() {
    ...
    int err = itl::qmr(A, x, b, pc, iter); // no explicit Accumulator -- must be unaffected
    if (err != 0) {
        std::cerr << "QMR (default Accumulator) did not converge, err=" << err << '\n';
        return false;
    }
    ...
    for (std::size_t i = 0; i < 3; ++i) {
        if (std::abs(r(i) - b(i)) > 1e-8) {
            std::cerr << "QMR (default Accumulator) residual component " << i
                      << " off: " << r(i) << " vs " << b(i) << '\n';
            return false;
        }
    }
...
// --- Edge case: QMR on a trivial 1x1 system ---
bool qmr_1x1_ok() {
    ...
    if (err != 0) {
        std::cerr << "QMR (1x1) did not converge, err=" << err << '\n';
        return false;
    }
    if (std::abs(x(0) - 0.5) > 1e-8) {
        std::cerr << "QMR (1x1) wrong solution: " << x(0) << " vs 0.5\n";
        return false;
    }
    return true;
}

// --- The actual point: quire accumulation vs naive posit32 ---
bool qmr_quire_beats_naive_posit32() {
    ...
    itl::qmr(A, x_naive, b, pc, iter_naive); // default Accumulator = naive posit32

    vec::dense_vector<Posit> x_quire(n, Posit(0.0));
    itl::basic_iteration<Posit> iter_quire(b, 200, Posit(1e-6));
    itl::qmr<mat::dense2D<Posit>, vec::dense_vector<Posit>, vec::dense_vector<Posit>,
    ...
    // The claim under test: quire accumulation should not be worse, and on
    // this tridiagonal system should measurably improve, final residual
    // accuracy vs naive same-precision accumulation.
...
int main() {
    ...
    if (!qmr_default_accumulator_ok())      ++failures;
    if (!qmr_1x1_ok())                      ++failures;
    if (!qmr_quire_beats_naive_posit32())   ++failures;

    if (failures == 0) std::cout << "test_qmr_quire passed\n";
