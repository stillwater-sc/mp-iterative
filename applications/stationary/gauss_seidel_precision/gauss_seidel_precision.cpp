// gauss_seidel_precision: Gauss-Seidel iteration on a shifted 1D Poisson
// system (tridiagonal, diagonally dominant), compared across number types AND
// accumulator strategies (issue #4). Companion to jacobi_precision.
//
// The iteration runs through mtl::itl::smoother::gauss_seidel<Matrix,
// Accumulator> (accumulator-aware since mtl5 #266): the number system is one
// template parameter, the accumulation strategy for the row sums is another.
// Gauss-Seidel updates in place, so within a sweep row i reads the entries
// already updated earlier in the same sweep -- a different error-propagation
// regime than Jacobi. The accumulator governs only the rounding of each row
// sum. Strategies:
...
// Gauss-Seidel on (4I - tridiag(1, 0, 1)) x = b with b = ones; the solution
// is NOT exactly representable, so the converged residual exposes the rounding
// floor of each number type. Returns ||A*x - b||_inf evaluated in double.
template <typename T, typename Accumulator = void>
double gauss_seidel_residual(std::size_t n, std::size_t iterations) {
    ...
    mtl::itl::smoother::gauss_seidel<Sparse, Accumulator> smoother(A);
...
void report(const std::string& name, std::size_t n, std::size_t iterations) {
    ...
    print_cell(gauss_seidel_residual<T>(n, iterations));   // naive

    if constexpr (has_fused_multiply_add<T>)
        print_cell(gauss_seidel_residual<T, mtl::math::fma_accumulator<T>>(n, iterations));
    else
        print_na();

    using Quire = typename quire_of<T>::type;
    if constexpr (!std::is_void_v<Quire>)
        print_cell(gauss_seidel_residual<T, Quire>(n, iterations));
    else
        print_na();
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Gauss-Seidel on shifted 1D Poisson, n = " << n
